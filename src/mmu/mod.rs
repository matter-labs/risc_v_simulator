...
use crate::cycle::status_registers::{TrapReason, SATPRegister};
use crate::utils::*;
...
pub trait MMUImplementation<M: MemorySource, MTR: MemoryAccessTracer> {
    ...
    fn map_virtual_to_physical(&self, virt_address: u32, mode: Mode, access_type: AccessType, aux_source: &mut Self::AuxilarySource, trap: &mut u32) -> u64;
}
...
impl PageTableEntry {
    ...
    pub const fn is_valid(&self) -> bool {
        self.test_bit(EntryBit::Valid) != 0 && 
            !(
                self.test_bit(EntryBit::Write) != 0 &&
                self.test_bit(EntryBit::Read) == 0
            )
    }

    #[must_use]
    #[inline(always)]
    pub fn is_valid_for_access_type_in_privilege(&self, access_type: AccessType, privilege: Mode) -> bool {
        ...
        invalid = invalid ||  access_type == AccessType::Store && self.test_bit(EntryBit::Write) == 0;
        invalid = invalid ||  access_type == AccessType::Instruction && self.test_bit(EntryBit::Execute) == 0;
    ...
    pub fn is_valid_for_ad_flags(&self, access_type: AccessType) -> bool {
        ...
        invalid = invalid ||  access_type == AccessType::Store && self.test_bit(EntryBit::Dirty) == 0;
...
> MMUImplementation<M, MTR> for NoMMU {
    ...
    #[must_use]
    #[inline(always)]
    fn map_virtual_to_physical(&self, virt_address: u32, _mode: Mode, _access_type: AccessType, _aux_source: &mut Self::AuxilarySource, _trap: &mut u32) -> u64 {
        virt_address as u64
    }
}
...
impl<
    M: MemorySource,
    MTR: MemoryAccessTracer
> MMUImplementation<M, MTR> for SimpleMMU {
    type AuxilarySource = MemoryImplementation<M, MTR>;
    ...
    fn map_virtual_to_physical(&self, virt_address: u32, mode: Mode, access_type: AccessType, aux_source: &mut Self::AuxilarySource, trap: &mut u32) -> u64 {
        ...
        if should_translate == false {
            ...
            let physical_address_candidates: [u64; 2] = [
                (ppns[1] as u64) << 22 | (ppns[0] as u64) << 12 | get_bits_and_align_right(virt_address, 0, SV32_PAGE_SIZE_LOG_2) as u64,
                (ppns[1] as u64) << 22 | get_bits_and_align_right(virt_address, 0, SV32_SUPERPAGE_SIZE_LOG_2) as u64
            ];
