...
use crate::{cycle::state::RiscV32State, abstractions::memory::VectorMemoryImpl, mmio::MMIOImplementation};
...
pub fn run_simple_simulator(
    ...
    for (word, dst) in os_image.array_chunks::<4>().zip(memory.inner[((DEFAULT_ENTRY_POINT/4) as usize)..].iter_mut()) {
